...
add_filter( 'get_the_archive_title',       'hybrid_archive_title_filter',       5, 3  );
 ...
 * @return string
 */
function hybrid_archive_title_filter( $title, $original_title, $prefix ) {
	...
	return $prefix . ' <span>' . apply_filters( 'hybrid_archive_title', $title ) . '</span>';
}
...
function hybrid_untitled_post( $title ) {
	...
	if ( ! $title && ! is_singular() && in_the_loop() && ! is_admin() )
		$title = esc_html__( '(Untitled)', 'wuqi' );
